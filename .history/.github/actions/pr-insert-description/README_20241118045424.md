# PR attach pr body

## 概要

このアクションは、プルリクエストの説明に特定のテキストを挿入するためのものです。指定されたラベルが存在する場合にのみ動作します。

## 使い方

以下は、アクションを使用する際の例です。

```yaml
name: 'Update PR Description'
on:
  pull_request:
    types: [opened, edited, labeled, synchronize]
  push:
    branches:
      - '**'

jobs:
  update-pr-description:
    runs-on: ubuntu-latest
    steps:
      - name: 'Checkout repository'
        uses: actions/checkout@v2

      - name: 'Insert text between comment tags in PR description if label is present'
        uses: ./.github/actions/pr-insert-description
        with:
          label: 'your-label'
          text: |
            This is the text to insert.
          # keyword: 'hoge'  # Optional, will use 'default-keyword' if omitted
```

## 入力パラメーター

| パラメーター名 | 必須 | デフォルト値 | 説明 |
| ---------------- | ---- | ------------ | ---- |
| github_token     | はい | なし         | リポジトリのGitHub Token（${{ secrets.GITHUB_TOKEN }}）を指定します。 |
| label            | はい | なし         | PRに付与されているラベル名を指定します。 |
| text             | はい | なし         | PRの説明に挿入するテキストを指定します。 |
| keyword          | いいえ | 'ADD-PR-DESCRIPTION' | コメントタグを識別するためのキーワードを指定します。 |

## その他

このアクションを利用することで、プルリクエストのレビュー時にチェックリスト項目の確認を自動化し、未チェック項目の見逃しを防ぐことができます。対象となるチェックリストは、PR本文の中で `<!-- ADD-PR-DESCRIPTION:START -->` と `<!-- ADD-PR-DESCRIPTION:END -->` で囲まれた範囲内の内容のみを対象とします。

### 例

```markdown
<!-- ADD-PR-DESCRIPTION:START -->
- [ ] チェックリスト項目1
- [ ] チェックリスト項目2
<!-- ADD-PR-DESCRIPTION:END -->
```

## リポジトリにブランチの保護ルールを設定する

1. リポジトリの設定にアクセスする
2. Settings（設定） タブをクリックする
3. 左側のサイドバーで Code and automation セクションの Branches をクリックする
4. new ruleset をクリックする
5. Add branch rule（ブランチルールを追加） ボタンをクリックする
6. ルール名を記入 (例: PR Checklist Validator)
7. Enforcement status を Enabled にする
8. Target branchesのadd target で対象ブランチを指定する
9. 「Require status checks to pass」にcheckを入れる
10. 「Require branches to be up to date before merging」にcheckを入れる
11. 「Status checks that are required」の 「+ Add checks」を押下
12. 導入したworkflowの名称を選択(例:validate-checklist)
13. 「Save changes」を押下
