...
このアクションを利用することで、プルリクエストのレビュー時にテキストの挿入を自動化し、未チェック項目の見逃しを防ぐことができます。対象となるテキストは、PR本文の中で `<!-- PR-INSERT-DESCRIPTION:START -->` と `<!-- PR-INSERT-DESCRIPTION:END -->` で囲まれた範囲内の内容のみを対象とします。
